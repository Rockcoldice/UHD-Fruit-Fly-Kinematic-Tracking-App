...
void VideoData::DistanceCalc() {
	...
	for (int i = 0; i <= size; i++) {
		...
		if (i%FPM == 0 && i != 0)
		{
			...
			cout << "Vector Value: " << MinuteVector[MinCounter].InnerTime << endl;
			framecounter = 0;
			MinCounter++;
		}
...
void VideoData::WriteToFile(string FileName, string VideoName) {
	...
	{	
		...
		myFile << "Minutes,Distance [cm],Avg Velocity [cm/s],Time in Inner Arena [s],Active Time [s],InActive Time [s]" << endl;
		for (int i = 0; i <= MinSize; i++) {
			...
			myFile << MinuteVector[i].InActiveTime << endl;
		}
		...
		for (int i = 0; i <= FrameSize; i++) {
			...
			myFile << FrameVector[i].Velocity << endl;
		}
	}
	else {
